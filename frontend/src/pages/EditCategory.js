...
export default function EditCategory() {
  ...
  useEffect(() => {
    // update selected category if data is changed (i.e. new field is added)
    if (selectedCategory) {
  ...
  useEffect(() => {
    ...
    const findFields = () => {
      const tempFields = {};
      selectedCategory.field_ids.forEach(
        (currId) => (tempFields[currId] = fields[currId])
      );
    ...
    if (selectedCategory) {
      ...
      if (
        ...
        setSubDeletion({
          ...subDeletion,
          allowSubChoice: false,
          subWarning: "Are you sure you want to delete this category?",
          deleteSubs: false,
          showSubs: false,
        });
              ...
              <>
                ...
                <ul className="mb-8">
                  {Object.keys(displayedFields).length > 0 ? (
                    Object.values(displayedFields).map((item) => (
                      <div
                        className="flex flex-wrap gap-3 mt-2 mb-6 lg:mb-3 items-center"
                        key={`group-${item.id}`}
                      >
                        <li key={item.id}>
                          {item.name} ({item.type})
                ...
                {addingExistingField && (
                  <form onSubmit={addExistingField}>
                    ...
                    <div className="self-center flex gap-4 my-4 items-center">
                      <ActionButton
                        onClick={() => setAddingExistingField(false)}
                        color="red"
                        size="lg"
                        type="button"
                      >
                ...
                {addingField && (
                  <form onSubmit={addField}>
                    ...
                    <input
                      type="text"
                      className="mb-4 w-full rounded-md border-gray-300 shadow-sm
                                      focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                      id="name"
                      value={newField}
                ...
                {!addingField && !addingExistingField && (
                  <div className="flex justify-center lg:justify-start gap-8">
                    <ActionButton
                      onClick={() => setAddingExistingField(true)}
                      size="lg"
                    >
                      Add Existing Field
                    </ActionButton>
                    ...
                    {subDeletion.allowSubChoice && (
                      <>
                        ...
                        <input
                          ...
                          id="subs"
                          className="mx-4"
                          checked={subDeletion.deleteSubs}
                          onChange={toggleSubDeletion}
                        />
                      </>
                    )}
