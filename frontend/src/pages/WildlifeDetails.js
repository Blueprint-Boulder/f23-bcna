                ...
                {wildlife && (
                    <>
                        ...
                        <div className="md:flex md:justify-around md:flex-wrap pb-10">
                            ...
                            {images.length >= 1 && <img src={`http://127.0.0.1:5000/api/get-image/${highlight}`} alt="" className="mb-5 min-w-80 h-64 object-cover" />}
                            <div className="flex gap-2 p-4 flex-wrap justify-center">
                            {images.length >= 2 && (
                                images.map(image => (
                                    <button key={wildlife[image]} onClick={() => setHighlight(wildlife[image])}>
                                                <img draggable="false" className={"object-cover min-w-32 h-16 md:w-36 md:h-16 rounded-md " + (highlight === wildlife[image] ? " border-sky-500 border-2" : "hover:border-sky-300 hover:border-2")}
