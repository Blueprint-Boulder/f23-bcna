...
def delete_wildlife():
    ...
    n_rows_deleted = db_helpers.delete(
        "DELETE FROM Wildlife WHERE id = ?", [wildlife_id]
    )
    if n_rows_deleted == 0:
    ...
    wildlife_images = db_helpers.select_multiple(
        "SELECT id FROM Images WHERE wildlife_id = ?", [wildlife_id]
    )
...
def search_wildlife_by_integer_field():
    ...
    field_info = db_helpers.select_one(
        "SELECT * FROM Fields WHERE id = ? AND type = 'INTEGER'", (field_id,)
    )
    if not field_info:
        return (
            jsonify({"error": "Invalid field ID or field is not of type INTEGER"}),
            400,
        )
    ...
    if exact_value is not None and (min_value is not None or max_value is not None):
        return (
            jsonify(
                {
                    "error": "Cannot specify exact_value together with min_value or max_value"
                }
            ),
            400,
        )
...
def edit_wildlife():
    ...
    wildlife_exists = db_helpers.select_one(
        "SELECT 1 FROM Wildlife WHERE id = ?", [wildlife_id]
    )
    ...
    other_fields = {
        ...
        if k
        not in ("name", "scientific_name", "category_id", "wildlife_id", "thumbnail_id")
    }

    # checking if category exists
    category_exists = db_helpers.select_one(
    ...
    valid_fields = db_helpers.select_multiple(
        f"""
    ...
    if not provided_field_names.issubset(valid_field_names):
        ...
        return (
            jsonify(
                {
                    "error": f"Provided fields not valid for category: {', '.join(invalid_fields)}"
                }
            ),
            400,
        )
    ...
    return (
        jsonify(
            {"message": "wildlife updated successfully", "wildlife_id": wildlife_id}
        ),
        201,
    )
    # ------------------------------------------------------------------------------------#
...
def create_wildlife():
    ...
    wildlife_with_name_exists = db_helpers.select_multiple(
    ...
    if wildlife_with_scientific_name_exists:
        return (
            jsonify(
                {
                    "message": f"Wildlife with scientific name {scientific_name} already exists"
                }
            ),
            400,
        )

    provided_nonimage_fields = {
        k: v
        for k, v in request.form.items()
        if k not in ("name", "scientific_name", "category_id", "thumbnail")
    }
    ...
    provided_field_names = set(
        list(provided_nonimage_fields.keys()) + list(request.files.keys())
    )
    if not provided_field_names.issubset(valid_field_names):
        ...
        return (
            jsonify(
                {
                    "error": f"The following fields are invalid for the category with ID {category_id}: {', '.join(invalid_field_names)}"
                }
            ),
            400,
        )
    ...
    if not valid_field_names.issubset(provided_field_names):
        ...
        return (
            jsonify(
                {"error": f"Missing required fields: {', '.join(missing_field_names)}"}
            ),
            400,
        )
    ...
    if not valid_image_field_names.issubset(request.files.keys()):
        ...
        return (
            jsonify(
                {
                    "error": f"The following are image fields, but you provided them as non-files: {', '.join(field_names_missing_file)}"
                }
            ),
            400,
        )
    ...
    if not valid_nonimage_field_names.issubset(provided_nonimage_fields.keys()):
        field_names_missing_value = (
            valid_nonimage_field_names - provided_nonimage_fields.keys()
        )
        return (
            jsonify(
                {
                    "error": f"The following are non-image fields, but you provided them as files: {', '.join(field_names_missing_value)}"
                }
            ),
            400,
        )
    ...
    return (
        jsonify(
            {"message": "Wildlife created successfully", "wildlife_id": wildlife_id}
        ),
        201,
    )
...
def search_wildlife_text_field():
    ...
    field_info = db_helpers.select_one(
        "SELECT type FROM Fields WHERE id = ?", (field_id,)
    )
...
def get_wildlife():
    ...
    for wildlife in all_wildlife:
        field_values = db_helpers.select_multiple(
            "SELECT * FROM FieldValues WHERE FieldValues.wildlife_id = ?",
            [wildlife["id"]],
        )
        cleaned_field_values = []
        for fv in field_values:
            field = db_helpers.select_one(
                "SELECT * FROM Fields WHERE id = ?", [fv["field_id"]]
            )
            if field["type"] == "TEXT":
                ...
                raise NotImplementedError(
                    "Unsupported field type '{}'".format(field["type"])
                )
...
def get_wildlife_by_id(wildlife_id):
    ...
    wildlife = db_helpers.select_one(
        "SELECT * FROM Wildlife WHERE id = ?", [wildlife_id]
    )
    if not wildlife:
        return jsonify({"error": "Wildlife not found"}), 404
    ...
    field_values = db_helpers.select_multiple(
        "SELECT * FROM FieldValues WHERE wildlife_id = ?", [wildlife_id]
    )
    ...
    try:
        category_row = db_helpers.select_one(
            "SELECT field_order FROM Categories WHERE id = ?", [wildlife["category_id"]]
        )
...
def create_field():
    ...
    field_exists = db_helpers.select_multiple(
        "SELECT 1 FROM Fields WHERE name = ?", [name]
    )
    if field_exists:
    ...
    if typ not in ("INTEGER", "TEXT", "ENUM", "IMAGE"):
        return (
            jsonify(
                {
                    "error": "Invalid field type. Allowed types are INTEGER, TEXT, ENUM, and IMAGE."
                }
            ),
            400,
        )

    field_id = db_helpers.insert(
        "INSERT INTO Fields (name, type) VALUES (?, ?)", [name, typ]
    )
...
def edit_field():
    ...
    field_exists = db_helpers.select_one(
        "SELECT 1 FROM Fields WHERE id = ?", (field_id,)
    )
    if not field_exists:
    ...
    if new_name:
        db_helpers.update(
            "UPDATE Fields SET name = ? WHERE id = ?", [new_name, field_id]
        )
...
def delete_field():
    ...
    if not row["association_exists"]:
        return (
            jsonify(
                {
                    "error": f"Field {field_id} is not associated with category {category_id}"
                }
            ),
            400,
        )
    ...
    db_helpers.delete(
        "DELETE FROM FieldsToCategories WHERE field_id = ? AND category_id = ?",
        (field_id, category_id),
    )
