  ...
  return (
    <>
      ...
      <section className="relative h-130 w-full overflow-hidden">
        <div 
          className="absolute inset-0 bg-cover bg-position-[25%_50%] bg-no-repeat"
          style={{ 
            backgroundImage: `url('/wildflower-hero.jpg')`,
          }}
        >
          {/* dark overlay */}
        ...
        <div className="flex max-w-[1500px] mx-auto gap-5">
          ...
          <main className="w-full min-w-0">
            <input
              ...
              placeholder={`Search ${butterflies.length.toLocaleString()} wildflowers…`}
            ...
            {filtered.length === 0 && (
              <p className="font-['Playfair_Display'] italic text-sand-400 mt-10 text-center">
                No wildflowers found for "{search}"
              </p>
            )}
          </main>
        </div>
